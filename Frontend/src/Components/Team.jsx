  ...
  return (
    ...
    >
      <div
        ...
        style={{
          transformStyle:
            window.innerWidth >= 768 ? "preserve-3d" : "flat",
        }}
      >
        {/* Dynamic Glow */}
        <div
          ref={glowRef}
          className="absolute w-[200px] h-[200px] bg-[var(--color-mint-blast)]/40 rounded-full blur-[80px] opacity-0 pointer-events-none z-0 hidden md:block mix-blend-screen"
        />
        ...
        <div
          className="relative w-full h-full flex flex-col items-center justify-end pb-6 md:pb-8 z-10"
          style={{
            transformStyle:
              window.innerWidth >= 768 ? "preserve-3d" : "flat",
          }}
        >
          ...
          <div
            ref={imageRef}
            className="absolute top-6 w-36 h-36 md:w-48 md:h-48 rounded-2xl overflow-hidden shadow-[0_15px_30px_rgba(0,0,0,0.6)] border-2 border-white/10 group-hover:border-[var(--color-mint-blast)] transition-colors duration-500"
          >
          ...
          <div
            ref={textRef}
            className="text-center bg-white/[0.02] backdrop-blur-xl w-[90%] p-4 md:p-6 rounded-2xl border border-white/10 shadow-[0_10px_30px_rgba(0,0,0,0.5)] transform-gpu"
          >
...
const Team = () => {
  ...
  return (
    <section
      id="team"
      className="py-20 md:py-32 bg-[var(--color-graphite-blue)] text-white relative overflow-hidden"
    >
      {/* Background Decor */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full">
        ...
        >
          <h2 className="text-4xl md:text-5xl font-black mb-6 tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-500">
            Our{" "}
            <span className="text-[var(--color-mint-blast)] drop-shadow-[0_0_15px_rgba(120,255,207,0.4)]">
              Core Team
            </span>
          </h2>
          ...
          <p className="mt-8 text-lg md:text-xl text-gray-400 max-w-2xl mx-auto font-medium">
            The driving force behind the Code Club, orchestrating the
            complexity into reality.
          </p>
        </motion.div>

        {/* Lead / Co-Lead Carousel */}
        <div className="relative group/carousel w-full">
          <div
            className="overflow-hidden p-4 -mx-4 md:mx-0"
            ref={emblaRef}
          >
            <div className="flex touch-pan-y">
              {teamMembers.map((member, idx) => (
                <div
                  key={idx}
                  className="flex-[0_0_85%] sm:flex-[0_0_45%] md:flex-[0_0_33.333%] lg:flex-[0_0_25%] min-w-0"
                >
                  <DeepProfileCard
                    member={member}
                    index={idx}
        ...
        {/* View All Members Button - BELOW THE CARDS */}
        <div className="flex justify-center mt-10 md:mt-12">
          <Link
            to="/team"
            className="inline-flex items-center gap-2 px-8 py-3 rounded-full border border-[var(--color-mint-blast)] text-[var(--color-mint-blast)] font-bold hover:bg-[var(--color-mint-blast)] hover:text-black transition-all duration-300 shadow-[0_0_15px_rgba(120,255,207,0.15)]"
          >
            View All Members
